...
def render_draft_mode_view(
    ...
    ACTIVE_DRAFT_ID = context.ACTIVE_DRAFT_ID

    ACTIVE_MANAGERS = context.ACTIVE_MANAGERS

    ACTIVE_MY_MANAGER_ID = context.ACTIVE_MY_MANAGER_ID

    SleeperClient = context.SleeperClient

    add_live_sale = context.add_live_sale

    auction_value_index = context.auction_value_index

    available_players = context.available_players

    calculate_context_valuation_adjustment = context.calculate_context_valuation_adjustment

    calculate_roster_aware_ceiling = context.calculate_roster_aware_ceiling

    compare_buy_vs_pass = context.compare_buy_vs_pass
    ...
    get_targeted_player_context = context.get_targeted_player_context

    live_calibration = context.live_calibration

    live_discretionary = context.live_discretionary

    live_open_spots = context.live_open_spots

    live_sales = context.live_sales

    live_team_setups = context.live_team_setups

    live_total_cash = context.live_total_cash

    market_value_index = context.market_value_index

    my_live_setup = context.my_live_setup

    my_need_profile = context.my_need_profile

    nomination_index = context.nomination_index

    nomination_recommendations = context.nomination_recommendations

    normalize_player_name = context.normalize_player_name

    optimal_roster_plan = context.optimal_roster_plan

    optimization_candidates = context.optimization_candidates

    player_value_index = context.player_value_index

    pool_result = context.pool_result

    projection_index = context.projection_index

    recommendation_index = context.recommendation_index

    recommendations = context.recommendations

    room_spend_index = context.room_spend_index

    sleeper_players = context.sleeper_players

    sync_next_sleeper_sale = context.sync_next_sleeper_sale

    team_need_profiles = context.team_need_profiles

    team_setups = context.team_setups

    threat_index = context.threat_index
